...
const Footer = () => {
  ...
  return (
    <footer className="bg-footer-bg text-text-inverse mt-16 sm:mt-20 md:mt-24">
      {/* Main Footer */}
      <div className="container mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-10 md:py-12 lg:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-8 md:gap-10 lg:gap-12">
          ...
          <div className="sm:col-span-2 lg:col-span-4">
            <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-5 text-primary">سفره‌شما</h3>
          ...
          <div className="sm:col-span-1 lg:col-span-2">
            <h4 className="font-bold text-base sm:text-lg mb-4 sm:mb-5 text-primary">دسترسی سریع</h4>
            <ul className="space-y-2 sm:space-y-3">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <Link 
                    href={link.href} 
                    className="text-text-muted hover:text-text-inverse text-sm sm:text-base transition-colors duration-200 hover:pr-2 block"
                  >
                    {link.name}
                  </Link>
                </li>
          ...
          {/* Categories Section */}
          <div className="sm:col-span-1 lg:col-span-2">
            <h4 className="font-bold text-base sm:text-lg mb-4 sm:mb-5 text-primary">دسته‌بندی محصولات</h4>
            <ul className="space-y-2 sm:space-y-3">
          ...
          <div className="sm:col-span-2 lg:col-span-4">
            <h4 className="font-bold text-base sm:text-lg mb-4 sm:mb-5 text-primary">ارتباط با ما</h4>
            <ul className="space-y-3 sm:space-y-4">
              <li className="flex items-start gap-3 text-text-muted text-sm sm:text-base">
            ...
            <div className="hidden sm:flex gap-3 mt-6">
              {socialLinks.map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  className="w-9 h-9 bg-gray-800 rounded-full flex items-center justify-center hover:bg-primary transition-colors hover:scale-110 duration-200"
                  aria-label={social.name}
                >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-col xs:flex-row gap-3 w-full sm:w-auto">
              <input
                ...
                className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm focus:outline-none focus:border-primary w-full xs:w-64"
              />
              <button className="px-6 py-2 bg-primary hover:bg-primary-hover rounded-lg text-sm font-medium transition-colors">
                عضویت
              </button>
            </div>
          </div>
